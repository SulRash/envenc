...
class AgentEmb(nn.Module):
    def __init__(self, envs):
        super().__init__()
        self.critic = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), 16384)),
            nn.Tanh(),
            layer_init(nn.Linear(16384, 16384)),
            nn.Tanh(),
            layer_init(nn.Linear(16384, 8192)),
            nn.Tanh(),
